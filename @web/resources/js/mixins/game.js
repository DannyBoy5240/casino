...
Vue.mixin({
  mounted() {
    ...
    if (!window.$multiplayerEventHandler) {
      ...
      Bus.$on('event:autoCashoutGameResult', (e) => {
           //validateMultiplayerAction(e.game, 'AutoCashoutGameResult');
           ...
           this.updateGameInstance((i) => {
                  i.playTimeout = false;
                  i.game.extendedId = e.game.id;
                 // i.game.extendedState = e.game.canBeFinished ? 'in-progress' : 'finished';
                  i.game.extendedState ='finished';
           });
       
           if (e && e.game.status !== 'cancelled') this.pushStats(e.game);
       //    $('.play-button').addClass('disabled');

         
           window.$gameRef.callback(e.game);
      ...
      //  this.updateGameInstance((i) => i.game.extendedState = 'finished');
       ...
       // window.$gameRef.callback(response);
          // if(successCallback != null) successCallback(e.game);
      });
  ...
  methods: {
    createGameInstance(id, ref) {
      ...
      if (ref.gameDataRetrieved) this.whisper('GameData', {api_id: id}).then((response) => {
       ...
       // console.log(this.gameInstance.game.data['wheel-double-black']);
    ...
    applyHouseEdge(payout) {
      ...
      return game.houseEdge ? (1 + ((payout - 1) * (1 - game.houseEdge / 100))).toFixed(2) : payout.toFixed(2);
    },
