  ...
  export default {
    ...
    mounted() {
      ...
      let update = setInterval(() => {
        if(this.gameInstance?.game?.data['dice-house-edge'] >= 0 && this.gameInstance.bet >= 0) {
          this.update();
    ...
    methods: {
      ...
      callback(response) {
        ...
        $('#circle').fadeIn('fast');
        $('#circle').css({
          left: 'calc(' + result.toFixed(2) + '% - 30px)',
          color: win ? 'green' : 'red'
        });
        ...
        $('#result').css({left: 'calc(' + result + '% - 30px)'});
      ...
      getProfit(min, max, houseEdge = null) {
        let payout, range;
        if (min === max) payout = 100.00;
        else {
          range = max - min;
          payout = 100.00 / range;
        }
        // if(houseEdge)
        //   payout *=  ((100 - houseEdge) / 100);
        // else
        // console.log('start');
        // console.log(min);
        // console.log(max); 
        // console.log(payout);
        // console.log(this.gameInstance.game.data['dice-house-edge']);
        // console.log(this.gameInstance.bet);
        // console.log('end');
        payout *=  ((100 - this.gameInstance.game.data['dice-house-edge']) / 100);
      ...
      gameDataRetrieved(data) {
       // this.getProfit()
      // const value = $('.game-dice .dice-slider').slider('value');
      // this.getProfit(this.currentTarget === 'lower' ? 0 : value, this.currentTarget === 'higher' ? 100 : value, data['dice-house-edge']);
      },
    }
  }
</script>
