  ...
  <div class="bots animate" v-if="data && status !== null">
    <div class="type">
      ...
      <dashboard-warning type="info">
        Bot will play crash game until they're stopped.
      </dashboard-warning>
  ...
  export default {
    ...
    methods: {
      ...
      start() {
        ...
        window.axios.post('/admin/bot/start', {game : 'crash'}).then(() => {
    ...
    created() {
      window.axios.post('/admin/bot/status', {game : 'crash'}).then(({ data }) => this.status = data.status);
      window.axios.post('/admin/bot/settings', {game : 'crash'}).then(({ data }) => this.data = data);
    },
    components: {
      WebIcon,
      DashboardWarning
