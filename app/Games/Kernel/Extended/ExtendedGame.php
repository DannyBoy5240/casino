...
abstract class ExtendedGame extends Game {
  ...
  public function process(Data $data) {
    ...
    if($this->metadata()->id() == 'crash') {
          if(is_float($data->autoCashoutMultiplier()) && $data->autoCashoutMultiplier() > 1.01) {
              Redis::zrem('crash', $game->_id);
              Log::info('autocashout multiplier je: ' . $data->autoCashoutMultiplier() * 100);
              Log::info('autocashout multiplier je: ' . round($data->autoCashoutMultiplier() * 100, 0));
              Redis::zadd('crash', round($data->autoCashoutMultiplier() * 100, 0), $game->_id);
          }
      }
