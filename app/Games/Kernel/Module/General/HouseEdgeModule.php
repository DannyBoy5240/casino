...
class HouseEdgeModule extends Module {
    ...
    public static function apply(Game $game, float $mul): float {
        ...
        if(Modules::get($game, false)->isEnabled($houseEdgeModule)) {
            // if($mul <= 1.00) $mul *= (1 - floatval(Modules::get($game, false)->get($houseEdgeModule, 'house_edge_option')) / 100);
            // else $mul = 1 + ($mul - 1) * (1 - floatval(Modules::get($game, false)->get($houseEdgeModule, 'house_edge_option')) / 100);
