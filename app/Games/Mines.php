...
class Mines extends ExtendedGame implements MultiplierCanBeLimited {
  ...
  public function data(): array {
    if (!Cache::has('phoenix:minesMultipliers:' . HouseEdgeModule::get($this, 1.00))) {
      $output = [];
      ...
      $calculateMultiplier = function ($mines, $diamonds) use ($nCr) {
        ...
        $percent = (100 - HouseEdgeModule::get($this, 1.00)) / 100;

        return 1 + ($percent * ($payout - 1));
      };
      ...
      Cache::forever('phoenix:minesMultipliers:' . HouseEdgeModule::get($this, 1.00), json_encode($output));
    }
    return json_decode(Cache::get('phoenix:minesMultipliers:' . HouseEdgeModule::get($this, 1.00)), true);
  }
  ...
  private function getGrid(Game $game, string $server_seed) {
    ...
    $bombs_grid = (new ProvablyFair($this, $server_seed, $game->id))->result()->result()[$bombs - 2];
    $grid = [];
