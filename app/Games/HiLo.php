...
class HiLo extends ExtendedGame implements MultiplierCanBeLimited {
  ...
  public function turn(\App\Models\Game $game, array $turnData): Turn {
    $card = (new ProvablyFair($this, $game->server_seed, $game->id))->result()->result()[$this->getTurn($game) - 1] + 1;
    ...
    if ($turnData['type'] === 'higher') $multiplier = 12.350 / (13 - ($current['slot']));
    else if ($turnData['type'] === 'lower') $multiplier =  12.350 / ($current['slot'] - 1);
    else if ($turnData['type'] === 'same') $multiplier = 16.83;



    $game->update([
      'multiplier' => HouseEdgeModule::apply($this, ($game->multiplier == 0 ? $multiplier : $game->multiplier * $multiplier))
    ]);
  ...
  public function isLoss(ProvablyFairResult $result, \App\Models\Game $game, array $turnData): bool {
    $card = (new ProvablyFair($this, $result->server_seed(), $game->id))->result()->result()[$this->getTurn($game)] + 1;
    $generated = $this->deck()[$card];
    $current = $this->deck()[$this->gameData($game)['current']];
