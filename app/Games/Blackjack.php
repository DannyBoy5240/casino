...
class Blackjack extends ExtendedGame {
    ...
    private function nextCard(\App\Models\Game $game, $hidden = false, string $server_seed = null) {
        ...
        $index = (new ProvablyFair($this, $server_seed == null ? $game->server_seed : $server_seed, $game->id))->result()->result()[$index];
    ...
    public function turn(\App\Models\Game $game, array $turnData): Turn {
        ...
        switch ($turnData['type']) {
            ...
            case 'split':
                ...
                if(array_key_exists(2, $this->gameData($game)['player']) || isset($this->gameData($game)['double']) ||
                   $this->gameData($game)['player'][0]['value'] != $this->gameData($game)['player'][1]['value'] )
            ...
            case 'double':
                if (isset($this->gameData($game)['double']) || isset($this->gameData($game)['insurance'])) 
                    return new ContinueGame($game, ['error' => true]);
                if(array_key_exists(2, $this->gameData($game)['player'])) return new ContinueGame($game, ['error' => true]);

                //if(($this->deck())[$this->gameData($game)['player'][0]][])
                ...
                $this->pushData($game, ['double' => true]);
                $updateRestore();


                return new ContinueGame($game, []);
            case 'insurance':
                if($this->gameData($game)['dealer'][0]['value'] != 'A' || array_key_exists($this->gameData($game)['player'], 2)) 
                    return new ContinueGame($game, ['error' => true]);
                if (isset($this->gameData($game)['insurance'])) return new ContinueGame($game, ['error' => true]);
                ...
                return new ContinueGame($game, []);
            case 'stand':
                if (isset($this->gameData($game)['double']) && !array_key_exists(2, $this->gameData($game)['player'])) 
                    return new ContinueGame($game, ['error' => true]);
                ...
                if($dealerScore == 21 && isset($this->gameData($game)['insurance'])) { 
                    $multiplier = 1;
                } else if ($playerScore == $dealerScore) {
                    if ($playerScore <= 21) $multiplier = 1;
                } else if ($playerScore > $dealerScore) {
                    if ($playerScore == 21 && $playerHandSize < 3) $multiplier = HouseEdgeModule::apply($this, 1.50);
